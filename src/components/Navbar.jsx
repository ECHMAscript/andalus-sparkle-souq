      ...
      <header className="sticky top-0 z-40 bg-background/85 backdrop-blur-md border-b border-border/60">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-3 sm:gap-6">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            >
              <Heart
                className={`size-4 ${favCount > 0 ? "text-destructive fill-destructive" : ""}`}
              />
              {favCount > 0 && (
            ...
            <button className="neo-pressable px-3 sm:px-4 py-2.5 flex items-center gap-1.5 sm:gap-2">
              <ShoppingBag className="size-4 text-primary" />
              <span className="text-xs font-semibold">2</span>
              <span className="hidden md:inline text-xs text-muted-foreground">€ 3,420</span>
            </button>
          </div>
        </div>
